...
server <-  function(input, output) {
    ...
    if (length(last_words) == 4) {
        ...
        if (nrow(filtered_bi >=3) ) { sep_five4 <- sepfive[which( sepfive$word4==last_words[4]), ]
        ...
        if (nrow(filtered_bi) <=3 )
        { filtered_one <- sepone[which(sepone$frequency >=10),] 
        
        sample_1 <- sample(filtered_one$feature, 3, replace=FALSE)
        
        
        sample_1 <- as.data.frame(sample_1)
        return(sample_1$sample_1[1:3]) 
    ...
    else { returned_object <- c("Please enter at least 4 words")
